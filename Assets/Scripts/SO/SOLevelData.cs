...
public class SOLevelData : ScriptableObject
{
    ...
    private void GenerateLevels()
    {
        ...
        for (int i = 4; i <= 20; i++)
        {
            ...
            // RoadsCount из заданного ренджа
            level.RoadsCount = UnityEngine.Random.Range(RoadsRange.x, RoadsRange.y + 1);

            // TowersPointCount >= RoadsCount, но в пределах своего ренджа
            ...
            // Количество стадий (прогрессия от min до max в зависимости от уровня)
            float progress = (float)(i - 4) / (20 - 4);
            int stagesCount = Mathf.RoundToInt(Mathf.Lerp(StagesRange.x, StagesRange.y, progress));

            // Доступные цвета расширяются каждые 2 уровня
            ...
            for (int s = 0; s < stagesCount; s++)
            {
                ...
                // Выбираем количество линий
                stage.LinesCount = UnityEngine.Random.Range(LinesRange.x, LinesRange.y + 1);

                // Если линий много (5 и выше), ограничиваем количество кубов (не выше 9)
                ...
                // Гарантируем, что min не больше max для Random.Range
